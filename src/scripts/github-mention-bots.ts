...
/** A row in the own-popup: a bot, or a user scraped out of GitHub's popup. */
type Item = Bot & { bot: boolean };
...
// ────────────────────────────────────────────────────────────────────────
const DEFAULTS: Omit<Config, 'seedVersion'> = {
  maxResults: 100,
...
const SORT_MODE: 'match' | 'top' | 'bottom' = 'match';
...
function botsHash(bots: Bot[]): string {
  const key = (bots || [])
    .map((b) => (b?.login ? b.login.toLowerCase() : ''))
    .filter(Boolean)
    .sort()
    .join(',');
...
function mergePinned(bots: Bot[]): Bot[] {
  const pinned = CUSTOM_BOTS.filter((b) => b?.login);
  const taken = new Set(pinned.map((b) => b.login.toLowerCase()));
  const rest = (bots || []).filter((b) => b?.login && !taken.has(b.login.toLowerCase()));
...
function activeMentionQuery(): { el: TextField; q: string } | null {
  const el = document.activeElement as TextField | null;
...
function matchBots(q: string): Bot[] {
...
function buildItemFromTemplate(bot: Bot, sample: HTMLElement): HTMLElement {
  ...
  const img = li.querySelector<HTMLImageElement>(
...
function buildItemStyled(bot: Bot): HTMLElement {
...
function buildItem(bot: Bot, sample: HTMLElement | null): HTMLElement {
  if (sample) {
    try {
...
function matchTier(login: string, text: string, q: string | null): number {
...
function nativeLogin(li: Element): string {
...
const menuSig = new WeakMap<Element, string>();

function reconcile(menu: HTMLElement) {
  const ctx = activeMentionQuery();
  const q = ctx ? ctx.q : null;
  let desired = q !== null ? matchBots(q) : [];

  const natives = [...menu.querySelectorAll<HTMLElement>('[role="option"]:not([data-gh-bot])')];
  ...
  if (menuSig.get(menu) === sig && (!desired.length || haveBots)) return;
  menuSig.set(menu, sig);

  for (const n of menu.querySelectorAll('[data-gh-bot]')) n.remove();
  if (!desired.length) return;

  const sample = natives[0] || null;

  if (SORT_MODE === 'top') {
    for (const bot of [...desired].reverse()) {
      menu.insertBefore(buildItem(bot, sample), menu.firstChild);
    }
  } else if (SORT_MODE === 'match') {
    for (const bot of desired) {
      const bt = matchTier(bot.login, `${bot.login} ${bot.name || ''}`, q);
      const ref =
        natives.find((n) => matchTier(nativeLogin(n), n.textContent || '', q) > bt) || null;
      menu.insertBefore(buildItem(bot, sample), ref); // ref=null → appended
    }
  } else {
    for (const bot of desired) menu.appendChild(buildItem(bot, sample));
  }
}
...
function activeMentionMenu(): HTMLElement | null {
  const el = document.activeElement;
  const id = el?.getAttribute?.('aria-controls');
  if (!id) return null;
  const menu = document.getElementById(id);
  if (menu?.matches(MENU_SELECTOR) && menu.offsetParent !== null) return menu;
  return null;
}
...
function sweep() {
  ...
  for (const menu of document.querySelectorAll<HTMLElement>(MENU_SELECTOR)) {
...
// ────────────────────────────────────────────────────────────────────────
let ownPopup: HTMLElement | null = null;
let ownItems: Item[] = [];
let ownIdx = 0;

function ensureOwnPopup(): HTMLElement {
...
function nativeMentionMenu(): HTMLElement | null {
  const id = document.activeElement?.getAttribute?.('aria-controls');
  if (id) {
    const m = document.getElementById(id);
    if (m?.matches(MENU_SELECTOR)) return m;
  }
  return (
    [...document.querySelectorAll<HTMLElement>(MENU_SELECTOR)].find(
...
function caretCoords(el: TextField): { x: number; y: number } {
  ...
  const caretAt = el.selectionStart ?? 0;
...
function scrapeUser(o: Element): Item | null {
...
function ownRow(it: Item, i: number): HTMLElement {
...
function ownHighlight(idx: number) {
  ownIdx = idx;
  if (!ownPopup) return;
  [...ownPopup.children].forEach((r, i) => {
    styleImp(r as HTMLElement, { background: i === idx ? 'rgba(128,128,128,.18)' : 'transparent' });
    if (i === idx) r.scrollIntoView({ block: 'nearest' });
  });
}
function ownMove(d: number) {
...
let lastOwnSig = '';
function unhideNative(native: HTMLElement | null) {
...
function sweepOwn() {
  ...
  const bots: Item[] = matchBots(ctx.q).map((b) => ({
    login: b.login,
    name: b.name,
    avatar: b.avatar,
    bot: true,
...
document.addEventListener(
  'mousedown',
  (e) => {
    if (ownVisible() && ownPopup && !ownPopup.contains(e.target as Node)) hideOwnPopup();
  },
  true,
);
...
function ghbOptions(menu: HTMLElement): HTMLElement[] {
  return [...menu.querySelectorAll<HTMLElement>('[role="option"]')].filter(
    (o) => o.offsetParent !== null,
  );
}
function ghbSetCursor(opts: HTMLElement[], idx: number) {
  opts.forEach((o, i) => {
    const on = i === idx;
...
function ghbCommit(opt: HTMLElement) {
...
function ghbActiveMenuWithBots(): HTMLElement | null {
  const active = activeMentionMenu();
  if (active?.querySelector('[data-gh-bot]')) return active;
  return (
    [...document.querySelectorAll<HTMLElement>(MENU_SELECTOR)].find(
