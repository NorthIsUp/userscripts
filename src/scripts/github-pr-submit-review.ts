...
type Action = {
  ...
  title: string;
  html: string;
  /** Submits a review event, which GitHub rejects on your own PR. */
  review?: boolean;
  run: (p: PullRequest) => void;
};

function onDiffRoute(): boolean {
  return /\/(files|changes)(\/|$)/.test(location.pathname);
}

function linkTo(re: RegExp): HTMLAnchorElement | null {
  const links = document.querySelectorAll<HTMLAnchorElement>('a[href]');
  for (const link of links) {
    if (re.test(link.getAttribute('href') || '')) return link;
  }
  return null;
}

function reviewDialog(): HTMLElement | null {
  return findByText<HTMLElement>(
    'dialog,[role=dialog]',
    (t) => /Finish your review/.test(t),
    false,
  );
}
...
function withDialog(p: PullRequest, key: string, fn: (dialog: HTMLElement) => void) {
  const openIt = () =>
    poll(
      () => document.querySelector<HTMLButtonElement>(NATIVE),
      (btn) => {
        btn.click();
...
function chooseEvent(dialog: HTMLElement, value: string): boolean {
...
function dialogSubmit(dialog: HTMLElement): HTMLButtonElement | null {
  const buttons = dialog.querySelectorAll<HTMLButtonElement>('button');
  for (const btn of buttons) {
    if (/^Submit review/.test((btn.textContent || '').trim())) return btn;
  }
  return null;
}
...
function compose(p: PullRequest, key: string, event: string) {
...
function approveNow(p: PullRequest) {
...
const selfCache: Record<string, boolean> = {};

function prAuthor(): string | null {
...
function nativeClose(): HTMLButtonElement | null {
  return findByText<HTMLButtonElement>('button', (t) => /^Close pull request$/.test(t));
}

function closePR(p: PullRequest) {
...
function makeButton(code: HTMLButtonElement, p: PullRequest, action: Action): HTMLButtonElement {
  ...
  const btn = code.cloneNode(true) as HTMLButtonElement;
...
function addButtons() {
  ...
  const code = findByText<HTMLButtonElement>('button', (t) => t === 'Code');
  ...
  for (const action of ACTIONS) row.appendChild(makeButton(code, p, action));
...
function resumeAfterReload() {
  ...
  ACTIONS.find((a) => a.key === key)?.run(p);
}
