...
(() => {
  ...
  for (const [d, v] of Object.entries(pruned)) {
    if (v !== "always" && Date.now() > v) { delete pruned[d]; dirty = true; }
  }
  ...
  const matchEntry = (allows) =>
    Object.keys(allows).find((d) => location.hostname === d || location.hostname.endsWith("." + d) || baseHost === d);
  ...
  const allowSite = (durSec) => { setAllow(baseHost, durSec); location.reload(); };
  ...
  const openConfig = () => {
    ...
    head.style.cssText = "display:flex;justify-content:space-between;align-items:center;gap:12px;font-weight:600;";
    ...
    if (cur) {
      site.append(`${location.hostname}: allowed`, btn("block again", () => { removeAllow(cur); location.reload(); }));
    } else {
    ...
    if (entries.length === 0) {
      ...
      for (const [domain, v] of entries) {
        ...
        const rm = btn("✕", () => {
          removeAllow(domain);
          if (domain === matchEntry({ [domain]: 1 }) && matchEntry({ [domain]: 1 })) location.reload();
          else openConfig();
        });
    ...
    cb.checked = isMuted();
    cb.onchange = () => {
      try { localStorage.setItem(MUTE_KEY, cb.checked ? "1" : "0"); } catch (e) {}
    };
  ...
  try { W.denyGeoConfig = openConfig; } catch (e) {}
  ...
  const notify = () => {
    ...
    const show = () => {
      ...
      node.append(msg, sep, btn("1d", () => allowSite(86400)), btn("always", () => allowSite("always")), btn("⚙", openConfig));
      Toastify({
        node,
        duration: 10000,
        close: true,
  ...
  const deny = (_ok, err) => {
    notify();
    if (typeof err === "function") setTimeout(() => { try { err(ERR); } catch (e) {} }, 0);
  };

  Object.defineProperty(W.navigator, "geolocation", {
    value: {
      getCurrentPosition: (ok, err) => deny(ok, err),
      watchPosition: (ok, err) => { deny(ok, err); return 0; },
      clearWatch: () => {},
    },
    configurable: false,
  });
