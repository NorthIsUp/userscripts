...
// @namespace    https://github.com/NorthIsUp/userscripts/github-tokens-link
// @version      1.4.5
...
// @grant        none
// @updateURL    https://raw.githubusercontent.com/NorthIsUp/userscripts/dist/github-tokens-link.user.js
// @downloadURL  https://raw.githubusercontent.com/NorthIsUp/userscripts/dist/github-tokens-link.user.js
// ==/UserScript==
...
(function () {
    ...
    // then stop unless this script's own @match covers the page.
    {
      ...
      if (!announce({"name":"Code Helpers: GitHub — Quick API Tokens Link","namespace":"https://github.com/NorthIsUp/userscripts/github-tokens-link","version":"1.4.5"}, "^https:\\/\\/github\\.com\\/NorthIsUp\\/userscripts\\/releases.*$", ["^https:\\/\\/github\\.com\\/.*$"])) return;
    }
    ...
    function buildTokensItem(settingsLi) {
        ...
        for (const el of li.querySelectorAll('[id]'))
            el.removeAttribute('id');
        li.removeAttribute('id');
        // Also strip aria-labelledby since the referenced label id is gone.
        for (const el of li.querySelectorAll('[aria-labelledby]'))
     ...
     */
    function scan(root) {
        const candidates = new Set(root.querySelectorAll(MENU_SELECTOR));
        if (root instanceof Element && root.matches(MENU_SELECTOR))
            candidates.add(root);
