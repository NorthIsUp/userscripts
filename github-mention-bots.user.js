...
// @namespace    https://github.com/NorthIsUp/userscripts/github-mention-bots
// @version      3.2.5
...
// @updateURL    https://raw.githubusercontent.com/NorthIsUp/userscripts/dist/github-mention-bots.user.js
// @downloadURL  https://raw.githubusercontent.com/NorthIsUp/userscripts/dist/github-mention-bots.user.js
// ==/UserScript==
...
(function () {
  ...
  // then stop unless this script's own @match covers the page.
  {
    ...
    if (!announce({"name":"Code Helpers — GitHub @-mention Bots","namespace":"https://github.com/NorthIsUp/userscripts/github-mention-bots","version":"3.2.5"}, "^https:\\/\\/github\\.com\\/NorthIsUp\\/userscripts\\/releases.*$", ["^https:\\/\\/github\\.com\\/.*$"])) return;
  }
  ...
  function rowsEditor(opts) {
      ...
      const addRow = (item = {}) => {
          ...
          if (opts.previewFallback) {
              ...
              preview.onerror = () => {
                  if (preview)
                      preview.src = opts.previewFallback;
              };
          ...
          for (const col of opts.columns) {
              ...
              if (col.width)
                  input.style.flex = `0 0 ${col.width}`;
              if (preview && col.key === opts.previewKey) {
                  input.addEventListener('input', () => {
                      preview.src = input.value || opts.previewFallback;
                  });
              }
      ...
      return {
          el,
          read: () => [...rows.querySelectorAll('.row')].map((row) => {
              const out = {};
              for (const col of opts.columns) {
                  const input = row.querySelector(`[data-k="${col.key}"]`);
...
`;
  ...
  function activeMentionMenu() {
      ...
      if (menu?.matches(MENU_SELECTOR) && menu.offsetParent !== null)
          return menu;
      return null;
  }
  function sweep() {
  ...
  function ghbActiveMenuWithBots() {
      const active = activeMentionMenu();
      if (active?.querySelector('[data-gh-bot]'))
          return active;
